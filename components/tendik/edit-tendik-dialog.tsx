...
interface EditTendikDialogProps {
  open: boolean
...
export function EditTendikDialog({ open, onOpenChange, tendik, onSuccess }: EditTendikDialogProps) {
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState("")
  const [formData, setFormData] = useState(tendik)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.message || "Gagal mengupdate data Tendik")
      }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Edit Tendik</DialogTitle>
          <DialogDescription>Update informasi tenaga kependidikan</DialogDescription>
        </DialogHeader>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-sm font-medium mb-1 block">Unit Kerja</label>
                  <Input
                    name="department"
